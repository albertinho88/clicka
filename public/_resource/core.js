...
PrimeFaces.ajax = {VIEW_HEAD: "javax.faces.ViewHead", VIEW_BODY: "javax.faces.ViewBody", Utils: {loadStylesheets: function (b) {
            ...
            for (var d = 0; d < a.length; d++) {
                ...
                if (c.attr("method") === "post") {
                    ...
                    if (f.length > 0) {
                        ...
                        c.append('<input type="hidden" name="' + j + b + '" value="' + e + '" autocomplete="off" />')
                    }
                }
            }
            ...
            var o = (e.global === true || e.global === undefined) ? true : false, b = null, f = null;
            if (e.onstart) {
            ...
            var v = b.attr("action"), t = b.children("input[name*='javax.faces.encodedURL']"), g = [];
            var n = null;
            if (t.length > 0) {
                n = 'form[action="' + v + '"]';
                v = t.val()
            ...
            var m = PrimeFaces.ajax.Request.extractParameterNamespace(b);
            PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_REQUEST_PARAM, true, m);
            PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_SOURCE_PARAM, f, m);
            if (e.resetValues) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.RESET_VALUES_PARAM, true, m)
            }
            if (e.ignoreAutoUpdate) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.IGNORE_AUTO_UPDATE_PARAM, true, m)
            }
            if (e.skipChildren === false) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.SKIP_CHILDREN_PARAM, false, m)
            }
            var s = PrimeFaces.ajax.Request.resolveComponentsForAjaxCall(e, "process");
            if (e.fragmentId) {
                s.push(e.fragmentId)
            }
            var a = "@none";
            ...
            if (a !== "@none") {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_PROCESS_PARAM, a, m)
            }
            ...
            if (d.length > 0) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_UPDATE_PARAM, d.join(" "), m)
            }
            if (e.event) {
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.BEHAVIOR_EVENT_PARAM, e.event, m);
                var l = e.event;
                ...
                PrimeFaces.ajax.Request.addParam(g, PrimeFaces.PARTIAL_EVENT_PARAM, l, m)
            } else {
            ...
            if (e.partialSubmit && a.indexOf("@all") === -1) {
                ...
                if (!p) {
                    PrimeFaces.ajax.Request.addParamFromInput(g, PrimeFaces.VIEW_STATE, b, m);
                    PrimeFaces.ajax.Request.addParamFromInput(g, PrimeFaces.CLIENT_WINDOW, b, m);
                    PrimeFaces.ajax.Request.addParamFromInput(g, "dsPostWindowId", b, m);
                    PrimeFaces.ajax.Request.addParamFromInput(g, "dspwid", b, m)
                }
            } else {
            ...
            var r = {url: v, type: "POST", cache: false, dataType: "xml", data: c, portletForms: n, source: e.source, global: false, beforeSend: function (x, i) {
            ...
            var e = c.getAttribute("id"), b = PrimeFaces.ajax.Utils.getContent(c);
...
PrimeFaces.expressions.SearchExpressionFacade = {resolveComponentsAsSelector: function (c) {
        ...
        var k = PrimeFaces.expressions.SearchExpressionFacade.splitExpressions(l), c = [];
        if (k) {
                        ...
                        if (m.indexOf("@widgetVar(") == 0) {
                            ...
                            if (m.indexOf("@(") == 0) {
                                ...
                                for (var e = 0; e < b.length; e++) {
                                    var f = $(b[e]), a = f.data(PrimeFaces.CLIENT_ID_DATA) || f.attr("id");
...
PrimeFaces.widget = {};
PrimeFaces.widget.BaseWidget = Class.extend({init: function (a) {
        this.cfg = a;
        this.id = a.id;
...
PrimeFaces.widget.DeferredWidget = PrimeFaces.widget.BaseWidget.extend({renderDeferred: function () {
...
PrimeFaces.widget.AjaxStatus = PrimeFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        this.bind()
    }, bind: function () {
...
PrimeFaces.widget.Poll = PrimeFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        this.active = false;
