...
PrimeFaces.dialog.DialogHandler = {
    openDialog: function (e) {
        var g = this.findRootWindow(), j = e.sourceComponentId + "_dlg";
        if (g.document.getElementById(j)) {
            return
        }
        var i = e.sourceComponentId.replace(/:/g, "_") + "_dlgwidget",
                d = $('<div id="' + j + '" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-shadow ui-hidden-container ui-overlay-hidden" data-pfdlgcid="' + e.pfdlgcid + '" data-widgetvar="' + i + '"></div>')
        ...
        var c = d.find("iframe"), b = e.url.indexOf("?") === -1 ? "?" : "&", a = e.url.indexOf("pfdlgcid") === -1 ? e.url + b + "pfdlgcid=" + e.pfdlgcid : e.url, h = e.options.contentWidth || 640;
        c.width(h);
        c.on(
                "load",
                function () {
                    var p = $(this), l = p.contents().find("title"), o = false;
                    if (e.options.headerElement) {
                        var n = PrimeFaces.escapeClientId(e.options.headerElement), k = c.contents().find(n);
                        if (k.length) {
                            l = k;
                            o = true
                        }
                    ...
                    if (!p.data("initialized")) {
                        PrimeFaces.cw.call(
                                ...
                                i,
                                {
                                    ...
                                    onHide: function () {
                                        var s = this, r = this.content.children("iframe");
                                    ...
                                    modal: e.options.modal, resizable: e.options.resizable, hasIframe: true, draggable: e.options.draggable, width: e.options.width, height: e.options.height, minimizable: e.options.minimizable, maximizable: e.options.maximizable, headerElement: e.options.headerElement})
                    }
        ...
        var rootWindow = this.findRootWindow(), dlgs = $(rootWindow.document.body).children('div.ui-dialog[data-pfdlgcid="' + cfg.pfdlgcid + '"]').not("[data-queuedforremoval]"), dlgsLength = dlgs.length, dlg = dlgs.eq(dlgsLength - 1), parentDlg = dlgsLength > 1 ? dlgs.eq(dlgsLength - 2) : null, dlgWidget = rootWindow.PF(dlg.data("widgetvar")), sourceWidgetVar = dlgWidget.cfg.sourceWidgetVar, sourceComponentId = dlgWidget.cfg.sourceComponentId, dialogReturnBehavior = null, windowContext = null;
        dlg.attr("data-queuedforremoval", true);
        if (parentDlg) {
            var parentDlgFrame = parentDlg.find("> .ui-dialog-content > iframe").get(0), windowContext = parentDlgFrame.contentWindow || parentDlgFrame;
...
PrimeFaces.widget.AccordionPanel = PrimeFaces.widget.BaseWidget.extend({
        ...
        var a = this, b = this.panels.eq(c), d = b.prev();
        d.attr("aria-selected", false);
        ...
        var d = this.cfg.behaviors.tabChange, b = {params: [{name: this.id + "_newTab", value: a.attr("id")}, {name: this.id + "_tabindex", value: parseInt(a.index() / 2)}]};
        if (this.cfg.controlled) {
            var c = this;
        ...
        var a = this.panels.eq(b), d = this.cfg.behaviors.tabClose, c = {params: [{name: this.id + "_tabId", value: a.attr("id")}, {name: this.id + "_tabindex", value: parseInt(b / 2)}]};
        if (this.cfg.controlled) {
            var e = this;
...
PrimeFaces.widget.AutoComplete = PrimeFaces.widget.BaseWidget.extend({
        ...
        this.items.bind("mouseover", function () {
            ...
            var e = $(this), g = e.attr("data-item-value"), b = e.hasClass("ui-autocomplete-moretext");
            if (PrimeFaces.isIE(8)) {
                a.itemClick = true
            }
            if (b) {
        ...
        if (this.cfg.behaviors) {
            ...
            if (b) {
                var a = {params: [{name: this.id + "_moreText", value: true}]};
                b.call(this, a)
            }
        }
    }, removeItem: function (c, b) {
        var e = b.attr("data-token-value"), a = this.multiItemContainer.children("li.ui-autocomplete-token").index(b), d = this;
        ...
        if (this.panel.parent().is(this.jq)) {
            ...
            this.panel.position({my: this.cfg.myPos, at: this.cfg.atPos, of: this.cfg.multiple ? this.jq : this.input, collision: "flipfit"})
        }
        ...
        if (this.items.length) {
            ...
            this.items.filter(":not(.ui-autocomplete-moretext)").each(function (e) {
                var f = b.items.eq(e), g = f.data("item-group"), d = f.data("item-group-tooltip");
                if (b.currentGroup !== g) {
                    b.currentGroup = g;
...
PrimeFaces.widget.BlockUI = PrimeFaces.widget.BaseWidget.extend({
    ...
    bindTriggers: function () {
        ...
        for (var b = 0; b < this.block.length; b++) {
            var a = $(this.blocker[b]), c = $(this.content[b]);
            c.css({left: (a.width() - c.outerWidth()) / 2, top: (a.height() - c.outerHeight()) / 2, "z-index": ++PrimeFaces.zindex})
        }
        if (this.cfg.animate) {
            this.blocker.fadeIn()
        } else {
...
PrimeFaces.widget.Calendar = PrimeFaces.widget.BaseWidget.extend({
            ...
            this.cfg.beforeShow = function (g, i) {
                setTimeout(function () {
                    ...
                    if (a.cfg.showTodayButton === false) {
                        $(g).datepicker("widget").find(".ui-datepicker-current").hide()
                    }
                }, 1);
    ...
    configureLocale: function () {
        ...
        if (this.cfg.behaviors) {
            ...
            if (d) {
                var a = {params: [{name: this.id + "_month", value: c}, {name: this.id + "_year", value: b}]};
                d.call(this, a)
            }
        }
...
PrimeFaces.widget.Carousel = PrimeFaces.widget.DeferredWidget.extend({
    init: function (a) {
        ...
        this.itemsContainer.swipe({swipe: function (c, d) {
                if (d === "left") {
        ...
        if (b !== this.page && !this.itemsContainer.is(":animated")) {
            var a = this;
            this.itemsContainer.animate({left: -1 * (this.viewport.innerWidth() * b), easing: this.cfg.easing}, {duration: this.cfg.effectDuration, easing: this.cfg.easing, complete: function () {
                    a.page = b;
...
PrimeFaces.widget.ColumnToggler = PrimeFaces.widget.DeferredWidget.extend({
    init: function (a) {
        ...
        for (var e = 0; e < this.columns.length; e++) {
            var b = this.columns.eq(e), f = b.hasClass("ui-helper-hidden"), g = f ? "ui-chkbox-box ui-widget ui-corner-all ui-state-default" : "ui-chkbox-box ui-widget ui-corner-all ui-state-default ui-state-active", h = (f) ? "ui-chkbox-icon ui-icon ui-icon-blank" : "ui-chkbox-icon ui-icon ui-icon-check", k = b.children(".ui-column-title").text();
        ...
        if (this.cfg.behaviors) {
            ...
            if (b) {
                var a = e ? "VISIBLE" : "HIDDEN", d = {params: [{name: this.id + "_visibility", value: a}, {name: this.id + "_index", value: c}]};
                b.call(this, d)
            }
        }
            ...
            if (c) {
                this.cfg.update = function (h, g) {
                    if (this === g.item.parent()[0]) {
                        ...
                        var d = {params: [{name: a.id + "_reordered", value: true}, {name: a.id + "_widgetId", value: g.item.attr("id")}, {name: a.id + "_itemIndex", value: f}, {name: a.id + "_receiverColumnIndex", value: i}]};
                        if (g.sender) {
                            d.params.push({name: a.id + "_senderColumnIndex", value: g.sender.parent().children().index(g.sender)})
                        }
                        c.call(a, d)
                    }
                }
...
PrimeFaces.widget.DataGrid = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        var c = this, b = {source: this.id, update: this.id, process: this.id, formId: this.cfg.formId, params: [{name: this.id + "_pagination", value: true}, {name: this.id + "_first", value: d.first}, {name: this.id + "_rows", value: d.rows}], onsuccess: function (g, e, f) {
                PrimeFaces.ajax.Response.handle(g, e, f, {widget: c, handle: function (h) {
                        this.content.html(h)
                    }});
                return true
...
PrimeFaces.widget.DataScroller = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        if (this.cfg.mode === "document") {
            var c = $(window), b = $(document), d = this, a = "scroll." + this.id;
        ...
        var b = this, a = {source: this.id, process: this.id, update: this.id, global: false, params: [{name: this.id + "_load", value: true}, {name: this.id + "_offset", value: this.cfg.offset}], onsuccess: function (e, c, d) {
...
PrimeFaces.widget.DataTable = PrimeFaces.widget.DeferredWidget.extend({
    SORT_ORDER: {ASCENDING: 1, DESCENDING: -1, UNSORTED: 0}, init: function (a) {
        ...
        for (var c = 0; c < this.sortableColumns.length; c++) {
            var e = this.sortableColumns.eq(c), g = e.children("span.ui-sortable-column-icon"), d = null, a = e.attr("aria-label");
            if (e.hasClass("ui-state-active")) {
                ...
                if (f.cfg.multiSort) {
                    f.addSortMeta({col: e.attr("id"), order: d})
                }
            } else {
        ...
        this.sortableColumns.on("mouseenter.dataTable", function () {
            ...
            PrimeFaces.clearSelection();
            var k = $(this), h = k.data("sortorder"), i = (h === f.SORT_ORDER.UNSORTED) ? f.SORT_ORDER.ASCENDING : -1 * h, m = l.metaKey || l.ctrlKey || j;
            if (f.cfg.multiSort) {
                if (m) {
                    f.addSortMeta({col: k.attr("id"), order: i});
                    f.sort(k, i, true)
                } else {
                    f.sortMeta = [];
                    f.addSortMeta({col: k.attr("id"), order: i});
        ...
        if (b && this.cfg.reflow) {
            ...
            b.change(function (j) {
                var k = $(this).val().split("_"), i = f.sortableColumns.eq(parseInt(k[0])), h = parseInt(k[1]);
                i.data("sortorder", h);
        ...
        this.tbody.off("focus.dataTable blur.dataTable change.dataTable", b).on("focus.dataTable", b, null, function () {
            ...
            var d = c.tbody.find(b).filter(":checked"), e = d.parent().next();
        ...
        if (this.cfg.nativeElements) {
            ...
            this.tbody.off("mouseover.dataTable mouseover.dataTable click.dataTable", a).on("mouseover.dataTable", a, null, function () {
                ...
                var e = $(this), d = e.hasClass("ui-state-active");
                if (d) {
        ...
        $(document).off(c, d).on(c, d, null, function (i) {
            ...
            if (f.cfg.selectionMode && j.hasClass("ui-datatable-selectable")) {
                ...
                if (f.cfg.editMode === "cell") {
                    var h = $(i.target), g = h.is("td.ui-editable-column") ? h : h.parents("td.ui-editable-column:first");
        ...
        this.scrollBody.on("scroll.dataTable", function () {
            ...
            if (c.shouldLiveScroll) {
                var f = Math.ceil(this.scrollTop), e = this.scrollHeight, d = this.clientHeight;
        ...
        if (!this.columnWidthsFixed) {
            ...
            if (this.cfg.scrollable) {
                this.scrollHeader.find("> .ui-datatable-scrollable-header-box > table > thead > tr > th").each(function () {
                    var h = $(this), e = h.index(), f = h.width();
                    h.width(f);
                ...
                var b = this.jq.find("> .ui-datatable-tablewrapper > table > thead > tr > th"), a = b.filter(":visible"), c = b.filter(":hidden");
        ...
        var b = this, a = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_scrolling", value: true}, {name: this.id + "_skipChildren", value: true}, {name: this.id + "_scrollOffset", value: this.scrollOffset}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (e, c, d) {
                PrimeFaces.ajax.Response.handle(e, c, d, {widget: b, handle: function (f) {
        ...
        var c = this, b = {source: this.id, update: this.id, process: this.id, formId: this.cfg.formId, params: [{name: this.id + "_pagination", value: true}, {name: this.id + "_first", value: d.first}, {name: this.id + "_rows", value: d.rows}, {name: this.id + "_skipChildren", value: true}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (g, e, f) {
                PrimeFaces.ajax.Response.handle(g, e, f, {widget: c, handle: function (h) {
                        this.updateData(h);
        ...
        var b = this, a = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_pagination", value: true}, {name: this.id + "_loadlazycache", value: true}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (e, c, d) {
                PrimeFaces.ajax.Response.handle(e, c, d, {widget: b, handle: function (f) {}});
                return true
            }};
        PrimeFaces.ajax.Request.handle(a)
    }, sort: function (d, a, f) {
        var e = this, b = {source: this.id, update: this.id, process: this.id, params: [{name: this.id + "_sorting", value: true}, {name: this.id + "_skipChildren", value: true}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (i, g, h) {
                PrimeFaces.ajax.Response.handle(i, g, h, {widget: e, handle: function (j) {
                        this.updateData(j)
                    }});
                return true
                ...
                if (!m.validationFailed) {
                    ...
                    if (!f) {
                        ...
                        for (var k = 0; k < g.length; k++) {
                            var h = $(g.get(k)), n = h.attr("aria-label");
                    ...
                    var l = d.find(".ui-sortable-column-icon"), o = d.attr("aria-label");
        ...
        var c = this, a = {source: this.id, update: this.id, process: this.id, formId: this.cfg.formId, params: [{name: this.id + "_filtering", value: true}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (f, d, e) {
                PrimeFaces.ajax.Response.handle(f, d, e, {widget: c, handle: function (g) {
                        this.updateData(g);
                        if (this.cfg.scrollable) {
                            this.alignScrollBody()
                        }
        ...
        var e = $(b), d = this.getRowMeta(e), a = e.hasClass("ui-state-highlight");
        if (f === "single" || !a) {
            this.unselectAllRows()
        }
        ...
        if (this.cursorIndex !== null) {
            var g = this.cursorIndex, a = g > this.originRowIndex ? c.slice(this.originRowIndex, g + 1) : c.slice(g, this.originRowIndex + 1);
            a.each(function (h, j) {
        ...
        var c = this.tbody.children("tr.ui-state-highlight"), a = this.isCheckboxSelectionEnabled(), e = this.isRadioSelectionEnabled();
        ...
        var d = b.closest("tr"), g = this.getRowMeta(d).index, f = b.hasClass("ui-icon"), e = b.children("span"), a = f ? b.hasClass("ui-icon-circle-triangle-s") : b.children("span").eq(0).hasClass("ui-helper-hidden"), c = this;
        ...
        var c = this, e = this.getRowMeta(d).index, a = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_rowExpansion", value: true}, {name: this.id + "_expandedRowIndex", value: e}, {name: this.id + "_encodeFeature", value: true}, {name: this.id + "_skipChildren", value: true}], onsuccess: function (h, f, g) {
                PrimeFaces.ajax.Response.handle(h, f, g, {widget: c, handle: function (i) {
                        if (i && $.trim(i).length) {
        ...
        this.getExpandedRows().each(function () {
            ...
            var c = f.children("td");
            for (var b = 0; b < c.length; b++) {
                var d = c.eq(b), e = d.children(".ui-row-toggler");
                if (e.length > 0) {
        ...
        var b = a.children("div.ui-cell-editor"), d = b.children("div.ui-cell-editor-input"), c = b.children("div.ui-cell-editor-output");
        ...
        var b = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_encodeFeature", value: true}, {name: this.id + "_cellInfo", value: c}, {name: f, value: f}], onsuccess: function (k, i, j) {
                PrimeFaces.ajax.Response.handle(k, i, j, {widget: g, handle: function (l) {
        ...
        var g = this.getRowMeta(a.closest("tr")), e = a.children(".ui-cell-editor"), d = a.index(), c = g.index + "," + d, f = this;
        if (g.key) {
            c = c + "," + g.key
        }
        var b = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_encodeFeature", value: true}, {name: this.id + "_cellEditCancel", value: true}, {name: this.id + "_cellInfo", value: c}], onsuccess: function (j, h, i) {
                PrimeFaces.ajax.Response.handle(j, h, i, {widget: f, handle: function (k) {
        ...
        var f = a.closest("tr"), g = this.getRowMeta(f).index, b = f.hasClass("ui-expanded-row"), e = this, c = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_rowEditIndex", value: this.getRowMeta(f).index}, {name: this.id + "_rowEditAction", value: d}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (j, h, i) {
                PrimeFaces.ajax.Response.handle(j, h, i, {widget: e, handle: function (k) {
                        if (b) {
                            this.collapseRow(f)
                        }
                        this.updateRow(f, k)
        ...
        a.draggable({axis: "x", start: function (d, e) {
                e.helper.data("originalposition", e.helper.offset());
                if (b.cfg.liveResize) {
                    b.jq.css("cursor", "col-resize")
                } else {
                    var f = b.cfg.stickyHeader ? b.clone : b.thead, c = b.cfg.scrollable ? b.scrollBody.height() : f.parent().height() - f.height() - 1;
                ...
                if (b.cfg.liveResize) {
                    ...
                    b.resizerHelper.offset({left: d.helper.offset().left + d.helper.width() / 2, top: b.thead.offset().top + b.thead.height()})
                }
            }, stop: function (c, d) {
                d.helper.css({left: "", top: "0px"});
                if (b.cfg.liveResize) {
        ...
        if (this.hasBehavior("colResize")) {
            var a = {source: this.id, process: this.id, params: [{name: this.id + "_colResize", value: true}, {name: this.id + "_columnId", value: b.attr("id")}, {name: this.id + "_width", value: b.width()}, {name: this.id + "_height", value: b.height()}]};
        ...
        var d, f, k = null, e = null, g = null, n = (this.cfg.resizeMode === "expand"), o = this.thead.parent();
        if (this.hasColumnGroup) {
        ...
        if ((e > a && g > a) || (n && e > a)) {
            ...
            if (this.cfg.scrollable) {
                var j = this.theadClone.parent(), m = d.index();
                if (n) {
                    var i = this;
                    j.width(j.width() + k);
                    ...
                    if (this.hasColumnGroup) {
                        ...
                        if (this.footerCols.length > 0) {
                            var h = this.footerCols.eq(m), c = h.next();
                            h.width(e);
                            c.width(g)
                        }
                    }
        ...
        var a = this;
        $(this.jqId + " thead th").draggable({appendTo: "body", opacity: 0.75, cursor: "move", scope: this.id, cancel: ":input,.ui-column-resizer", start: function (b, c) {
                ...
                b.html(c.html());
                return b.get(0)
            }}).droppable({hoverClass: "ui-state-highlight", tolerance: "pointer", scope: this.id, over: function (b, c) {
                c.helper.data("droppable-column", $(this))
            }, drop: function (c, j) {
                var n = j.draggable, f = j.helper.data("drop-location"), g = $(this), e = null, l = null;
                var k = a.tbody.find("> tr:not(.ui-expanded-row-content) > td:nth-child(" + (n.index() + 1) + ")"), m = a.tbody.find("> tr:not(.ui-expanded-row-content) > td:nth-child(" + (g.index() + 1) + ")");
                if (a.tfoot.length) {
                    var b = a.tfoot.find("> tr > td"), e = b.eq(n.index()), l = b.eq(g.index())
                }
                if (f > 0) {
                    ...
                    if (a.cfg.scrollable) {
                        var h = $(document.getElementById(n.attr("id") + "_clone")), d = $(document.getElementById(g.attr("id") + "_clone"));
                        h.insertAfter(d)
                    }
                } else {
                    n.insertBefore(g);
        ...
        var a = this;
        this.tbody.sortable({placeholder: "ui-datatable-rowordering ui-state-active", cursor: "move", handle: "td,span:not(.ui-c)", appendTo: document.body, start: function (b, c) {
                c.helper.css("z-index", ++PrimeFaces.zindex)
            }, helper: function (g, h) {
                var d = h.children(), f = $('<div class="ui-datatable ui-widget"><table><tbody></tbody></table></div>'), c = h.clone(), b = c.children();
                ...
                var c = e.item.data("ri"), f = a.paginator ? a.paginator.getFirst() + e.item.index() : e.item.index();
                a.syncRowParity();
                var b = {source: a.id, process: a.id, params: [{name: a.id + "_rowreorder", value: true}, {name: a.id + "_fromIndex", value: c}, {name: a.id + "_toIndex", value: f}, {name: this.id + "_skipChildren", value: true}]};
        ...
        var b = this.tbody.children("tr.ui-widget-content"), d = this.paginator ? this.paginator.getFirst() : 0;
        ...
        var b = this.thead.parent(), f = b.offset(), d = $(window), c = this, e = "scroll." + this.id, a = "resize.sticky-" + this.id;
        ...
        this.stickyContainer.css({position: "absolute", width: b.outerWidth(), top: f.top, left: f.left, "z-index": ++PrimeFaces.zindex});
        ...
        d.off(e).on(e, function () {
            ...
            if (h > g.top) {
                c.stickyContainer.css({position: "fixed", top: "0px"}).addClass("ui-shadow ui-sticky");
                ...
                c.stickyContainer.css({position: "absolute", top: g.top}).removeClass("ui-shadow ui-sticky");
        ...
        var b = this, a = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_addrow", value: true}, {name: this.id + "_skipChildren", value: true}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (e, c, d) {
                PrimeFaces.ajax.Response.handle(e, c, d, {widget: b, handle: function (f) {
                        this.tbody.append(f)
                    }});
                return true
...
PrimeFaces.widget.FrozenDataTable = PrimeFaces.widget.DataTable.extend({
    setupScrolling: function () {
        ...
        this.scrollBody.scroll(function () {
            var g = c.scrollBody.scrollLeft(), f = c.scrollBody.scrollTop();
            c.scrollHeaderBox.css("margin-left", -g);
            c.scrollFooterBox.css("margin-left", -g);
        ...
        var k = $("<table><tbody>" + c + "</tbody></table>"), m = k.find("> tbody > tr"), g = (e === undefined) ? true : e;
        if (g) {
            this.frozenTbody.children().remove();
            this.scrollTbody.children().remove()
        }
        var b = this.frozenTbody.children("tr:first"), h = b.length ? b.children("td").length : this.cfg.frozenColumns;
        for (var d = 0; d < m.length; d++) {
            var l = m.eq(d), a = l.children("td"), j = this.copyRow(l), f = this.copyRow(l);
        ...
        var d = $("<table><tbody>" + e + "</tbody></table>"), b = d.find("> tbody > tr"), c = b.children("td"), a = this.copyRow(b), f = this.copyRow(b), h = this.getTwinRow(g);
        ...
        var b = this.frozenThead.find("> tr > th.ui-resizable-column"), a = this.scrollThead.find("> tr > th.ui-resizable-column");
        ...
        var s = null, i = null, j = null, o = null, c = (this.cfg.resizeMode === "expand");
        if (this.hasColumnGroup) {
        ...
        var g = s.next();
        var m = s.index(), b = s.hasClass("ui-frozen-column-last");
        if (this.cfg.liveResize) {
        ...
        if (e) {
            var h = s.hasClass("ui-frozen-column"), k = h ? this.frozenTheadClone : this.scrollTheadClone, a = h ? this.frozenThead.parent() : this.scrollThead.parent(), d = k.parent(), v = h ? this.frozenFooterCols : this.scrollFooterCols, u = h ? this.frozenFooterTable : this.scrollFooterTable, f = this;
            if (c) {
                if (b) {
        ...
        var b = g.find("tr:first").children("td"), a = b.length, j = "", k = c ? "ui-resizable-column " + c : "ui-resizable-column";
        for (var d = 0; d < a; d++) {
    ...
    init: function (a) {
        ...
        a.on("keydown." + this.id, function (e) {
            ...
            if (e.which === $.ui.keyCode.TAB) {
                ...
                if (d.length) {
                    var g = d.filter(":first"), c = d.filter(":last"), h = null;
                    if (g.is(":radio")) {
        ...
        var a = this;
        this.jq.draggable({cancel: ".ui-dialog-content, .ui-dialog-titlebar-close", handle: ".ui-dialog-titlebar", containment: "document", stop: function (d, e) {
                if (a.hasBehavior("move")) {
                    var b = a.cfg.behaviors.move;
                    var c = {params: [{name: a.id + "_top", value: e.offset.top}, {name: a.id + "_left", value: e.offset.left}]};
                    b.call(a, c)
                }
            }})
        ...
        if (/(center|left|top|right|bottom)/.test(this.cfg.position)) {
            ...
            this.jq.position({my: "center", at: this.cfg.position, collision: "fit", of: window, using: function (h) {
                    var e = h.left < 0 ? 0 : h.left, f = h.top < 0 ? 0 : h.top, g = $(window).scrollTop();
                    if (c.cfg.absolutePositioned) {
                        f += g;
                        c.lastScrollTop = g
                    }
                    $(this).css({left: e, top: f})
                }})
        } else {
            var b = this.cfg.position.split(","), a = $.trim(b[0]), d = $.trim(b[1]);
        ...
        if (this.maximized) {
            ...
            this.content.css({width: "auto", height: this.jq.height() - this.titlebar.outerHeight() - a});
    ...
    init: function (a) {
        ...
        this.cfg.drop = function (c, d) {
            ...
            if (a.cfg.behaviors) {
                ...
                if (e) {
                    var b = {params: [{name: a.id + "_dragId", value: d.draggable.attr("id")}, {name: a.id + "_dropId", value: a.cfg.target}]};
                    e.call(a, b)
                }
            }
        }
...
PrimeFaces.widget.InputTextarea = PrimeFaces.widget.DeferredWidget.extend({
    init: function (a) {
        ...
        this.jq.on("keyup.inputtextarea-maxlength", function (d) {
            var c = a.normalizeNewlines(a.jq.val()), b = c.length;
        ...
        var d = this.normalizeNewlines(this.jq.val()), c = d.length;
        if (this.counter) {
        ...
        this.query = b;
        var c = this, a = {source: this.id, update: this.id, process: this.id, params: [{name: this.id + "_query", value: b}], onsuccess: function (f, d, e) {
                PrimeFaces.ajax.Response.handle(f, d, e, {widget: c, handle: function (g) {
                        this.panel.html(g);
        ...
        this.panel.css({left: a.left + b.left, top: a.top + b.top, width: this.jq.innerWidth(), "z-index": ++PrimeFaces.zindex})
    }, show: function () {
        this.alignPanel();
        this.panel.show()
    }, hide: function () {
...
PrimeFaces.widget.SelectOneMenu = PrimeFaces.widget.DeferredWidget.extend({
    init: function (a) {        
        ...
        var g = this, e = this.options.filter(":selected"), f = this.items.eq(e.index());
        ...
        this.focusInput.on("keydown.ui-selectonemenu", function (d) {
            ...
            switch (d) {
                ...
                default:
                    ...
                    var i = $(this).val(), c = null, h = g.metaKey || g.ctrlKey || g.shiftKey;
                    if (!h) {
        ...
        if (this.panel.parent().is(this.jq)) {
            ...
            this.panel.css({left: "", top: ""}).position({my: "left top", at: "left bottom", of: this.jq, collision: "flipfit"})
        }
    }, setLabel: function (b) {
        ...
        if (h === "") {
            ...
            for (var c = 0; c < this.options.length; c++) {
                var d = this.options.eq(c), b = this.cfg.caseSensitive ? d.text() : d.text().toLowerCase(), l = this.items.eq(c);
    ...
    init: function (b) {
        ...
        if (this.cfg.custom) {
            ...
            for (var e = 0; e < this.inputs.length; e++) {
                var c = this.inputs.eq(e), a = c.data("itemindex"), d = this.originalInputs.eq(a);
                c.val(d.val());
                if (d.is(":checked")) {
        ...
        this.enabledInputs.on("focus.selectOneRadio", function () {
            ...
            var i = $(this), f = i.parent().next(), g = a.enabledInputs.index(i), m = a.enabledInputs.length, l = $.ui.keyCode, j = h.which;
            switch (j) {
                case l.UP:
                case l.LEFT:
                    var c = (g === 0) ? a.enabledInputs.eq((m - 1)) : a.enabledInputs.eq(--g), k = c.parent().next();
                    i.blur();
                    a.unselect(f);
                    a.select(k);
                ...
                case l.DOWN:
                case l.RIGHT:
                    var d = (g === (m - 1)) ? a.enabledInputs.eq(0) : a.enabledInputs.eq(++g), b = d.parent().next();
                    i.blur();
                    a.unselect(f);
                    a.select(b);
                ...
                case l.SPACE:
                    if (!i.prop("checked")) {
                        a.select(f);
                        i.parent().next().addClass("ui-state-focus").removeClass("ui-state-active")
                    }
                    h.preventDefault();
                    break
                }
        ...
        if (c == null) {
            ...
            var a = this.inputs.eq(c), b = this.labels.filter("label[for='" + a.attr("id") + "']");
    ...
    bindEvents: function () {
        ...
        if (!this.cfg.disabled) {
            this.items.on("click.selectListbox", function (h) {
                ...
                var m = $(this), c = a.items.filter(".ui-state-highlight"), j = (h.metaKey || h.ctrlKey), b = (!j && c.length === 1 && c.index() === m.index());
                if (!h.shiftKey) {
                    if (!j) {
                        a.unselectAll()
                    }
                    ...
                    if (a.cursorItem) {
                        a.unselectAll();
                        var k = m.index(), n = a.cursorItem.index(), l = (k > n) ? n : k, g = (k > n) ? (k + 1) : (n + 1);
                        for (var f = l; f < g; f++) {
...
PrimeFaces.widget.SelectManyButton = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        this.buttons.on("mouseover", function () {
            ...
            var c = $(this), b = c.children(":checkbox");
            if (c.hasClass("ui-state-active")) {
                c.addClass("ui-state-hover")
            } else {
                c.removeClass("ui-state-hover")
...
PrimeFaces.widget.SelectCheckboxMenu = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        this.toggler = $('<div class="ui-chkbox ui-widget"><div class="ui-helper-hidden-accessible"><input type="checkbox" readonly="readonly"/></div><div class="ui-chkbox-box ui-widget ui-corner-all ui-state-default"><span class="ui-chkbox-icon ui-icon ui-icon-blank"></span></div></div>').appendTo(this.header);
        ...
        this.inputs.each(function () {
            var j = $(this), i = j.next(), d = j.is(":disabled"), k = j.is(":checked"), g = j.attr("title"), e = "ui-chkbox-box ui-widget ui-corner-all ui-state-default", h = "ui-selectcheckboxmenu-item ui-selectcheckboxmenu-list-item ui-corner-all", b = j.data("escaped");
            if (d) {
                e += " ui-state-disabled"
            }
            if (k) {
            ...
            var f = k ? "ui-chkbox-icon ui-icon ui-icon-check" : "ui-chkbox-icon ui-icon ui-icon-blank", h = k ? h + " ui-selectcheckboxmenu-checked" : h + " ui-selectcheckboxmenu-unchecked";
            var l = $('<li class="' + h + '"></li>');
            l.append('<div class="ui-chkbox ui-widget"><div class="ui-helper-hidden-accessible"><input type="checkbox" readonly="readonly"></input></div><div class="' + e + '"><span class="' + f + '"></span></div></div>');
            var c = $("<label></label>");
            if (b) {
                c.text(i.text())
            } else {
            ...
            l.find("> .ui-chkbox > .ui-helper-hidden-accessible > input").prop("checked", k);
        ...
        var a = this.itemContainer.children("li.ui-selectcheckboxmenu-item").filter(":visible"), b = this;
        a.each(function () {
            b.inputs.eq($(this).index()).prop("checked", true);
            b.check($(this).children(".ui-chkbox").children(".ui-chkbox-box"))
        });
        this.check(this.togglerBox);
        ...
        a.each(function () {
            ...
            b.uncheck($(this).children(".ui-chkbox").children(".ui-chkbox-box"))
        });
        ...
        if (!d.hasClass("ui-state-disabled")) {
            ...
            d.addClass("ui-state-active").children(".ui-chkbox-icon").removeClass("ui-icon-blank").addClass("ui-icon-check");
            d.closest("li.ui-selectcheckboxmenu-item").removeClass("ui-selectcheckboxmenu-unchecked").addClass("ui-selectcheckboxmenu-checked");
            if (c) {
        ...
        if (!d.hasClass("ui-state-disabled")) {
            ...
            d.removeClass("ui-state-active").children(".ui-chkbox-icon").addClass("ui-icon-blank").removeClass("ui-icon-check");
            d.closest("li.ui-selectcheckboxmenu-item").addClass("ui-selectcheckboxmenu-unchecked").removeClass("ui-selectcheckboxmenu-checked");
        ...
        var b = this.panel.css("position") == "fixed", c = $(window), a = b ? "-" + c.scrollLeft() + " -" + c.scrollTop() : null, d = this.panel.attr("style");
        this.panel.css({left: "", top: "", "z-index": ++PrimeFaces.zindex});
        if (this.panel.parent().attr("id") === this.id) {
            this.panel.css({left: 0, top: this.jq.innerHeight()})
        } else {
            this.panel.position({my: "left top", at: "left bottom", of: this.jq, offset: a})
        }
...
PrimeFaces.widget.Password = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        this.panel.css({left: "", top: "", "z-index": ++PrimeFaces.zindex}).position({my: "left top", at: "right top", of: this.jq})
    }, show: function () {
        if (!this.cfg.inline) {
            this.align();
            this.panel.fadeIn()
...
PrimeFaces.widget.SplitButton = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        this.menuButton.keydown(function (h) {
            ...
            switch (h.which) {
                case g.UP:
                    var f = c.menuitems.filter(".ui-state-hover"), d = f.length ? f.prevAll(":not(.ui-separator)") : null;
                    if (d && d.length) {
                    ...
                    break;
                case g.DOWN:
                    var f = c.menuitems.filter(".ui-state-hover"), i = f.length ? f.nextAll(":not(.ui-separator)") : c.menuitems.eq(0);
                    if (i.length) {
                        f.removeClass("ui-state-hover");
        ...
        if (this.menu.parent().is(this.jq)) {
            this.menu.css({left: 0, top: this.jq.innerHeight()})
        } else {
...
PrimeFaces.widget.MultiSelectListbox = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        var h = this.jq.children("div.ui-multiselectlistbox-listcontainer"), c = h.find(" > ul.ui-multiselectlistbox-list > li.ui-multiselectlistbox-item").filter('[data-value="' + g + '"]');
        c.addClass("ui-state-highlight");
    ...
    init: function (a) {
        ...
        if (!this.cfg.disabled) {
            ...
            if (this.cfg.editor) {
                ...
                var b = this.editor.children(".ui-inplace-save"), d = this.editor.children(".ui-inplace-cancel");
...
PrimeFaces.widget.LightBox = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        this.imageDisplay.load(function () {
            ...
            a.content.removeClass("ui-lightbox-loading").animate({width: d.width(), height: d.height()}, 500, function () {
                d.fadeIn();
                a.showNavigators();
                a.caption.slideDown()
            });
        ...
        var c = this, b = PrimeFaces.env.ios ? "touchstart." + this.id : "click." + this.id, a = "resize." + this.id;
        ...
        $(document.body).append('<div id="' + this.id + '_modal" class="ui-widget-overlay"></div>').children(this.jqId + "_modal").css({width: $(document).width(), height: $(document).height(), "z-index": this.panel.css("z-index") - 1})
    }, disableModality: function () {
        $(document.body).children(this.jqId + "_modal").remove()
...
PrimeFaces.widget.Menu = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        $(document.body).off(b).on(b, function (f) {
            ...
            var g = c.jq.offset();
            if (f.pageX < g.left || f.pageX > g.left + c.jq.width() || f.pageY < g.top || f.pageY > g.top + c.jq.height()) {
                c.hide(f)
            }
        });
        var a = "resize." + this.id;
...
PrimeFaces.widget.TieredMenu = PrimeFaces.widget.Menu.extend({
    init: function (a) {
    ...
    }, bindKeyEvents: function () {}, bindDocumentHandler: function () {
        ...
        var c = {my: "left top", at: "right top", of: b, collision: "flipfit"};
...
PrimeFaces.widget.Menubar = PrimeFaces.widget.TieredMenu.extend({
    showSubmenu: function (b, a) {
        ...
        if (b.parent().hasClass("ui-menu-child")) {
            ...
            c = {my: "left top", at: "left bottom", of: b, collision: "flipfit"}
        }
        a.css("z-index", ++PrimeFaces.zindex).show().position(c)
    }, bindKeyEvents: function () {
        var a = this;
        this.keyboardTarget.on("focus.menubar", function (b) {
            ...
            var g = !h.closest("ul").hasClass("ui-menu-child"), l = $.ui.keyCode;
            switch (j.which) {
                case l.LEFT:
                    if (g) {
    ...
    init: function (c) {
        ...
        if (!this.jq.hasClass("ui-menu-dynamic")) {
            if (this.jq.is(":not(:visible)")) {
                var a = this.jq.closest(".ui-hidden-container"), b = a.data("widget"), e = this;
                if (b) {
                    var d = PF(b);
                    if (d) {
...
PrimeFaces.widget.PlainMenu = PrimeFaces.widget.Menu.extend({
    init: function (a) {
        ...
        this.keyboardTarget.on("focus.menu", function () {
            ...
            var f = a.menuitemLinks.filter(".ui-state-hover"), g = $.ui.keyCode;
            switch (h.which) {
                case g.UP:
...
PrimeFaces.widget.ContextMenu = PrimeFaces.widget.TieredMenu.extend({
    init: function (b) {
        ...
        var a = this, d = (this.cfg.target === undefined);
        ...
        this.links.bind("click", function (c) {
            var b = $(c.target), d = b.hasClass("ui-submenu-link") ? b : b.closest(".ui-submenu-link");
            if (d.length) {
                return
            }
            a.hide()
        ...
        $(document.body).off(a).on(a, function (f) {
            var d = $(f.target), c = d.hasClass("ui-menuitem-link") ? d : d.closest(".ui-menuitem-link");
        ...
        var f = $(window), d = h.pageX, c = h.pageY, b = this.jq.outerWidth(), a = this.jq.outerHeight();
...
PrimeFaces.widget.MegaMenu = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        this.keyboardTarget.on("focus.megamenu", function (b) {
            ...
            var g = a.isRootLink(h), m = $.ui.keyCode;
            switch (j.which) {
                case m.LEFT:
                    if (g && !a.cfg.vertical) {
...
PrimeFaces.widget.PanelMenu = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        this.menuContent.off("keydown.panelmenu blur.panelmenu").on("keydown.panelmenu", function (k) {
            ...
            switch (k.which) {
                ...
                case j.RIGHT:
                    ...
                    break;
                case j.UP:
                    var i = null, c = b.focusedItem.prev();
                    if (c.length) {
                    ...
                    break;
                case j.DOWN:
                    var i = null, h = b.focusedItem.find("> ul > li:visible:first");
                    if (h.length) {
                        i = h
                    } else {
        ...
        var a = b.closest("ul").parent("li").next(), c = null;
        if (a.length) {
            c = a
        } else {
        ...
        if (this.cfg.stateful) {
            ...
            PrimeFaces.setCookie(this.stateKey, a, {path: "/"})
        }
    }, restoreState: function () {
        var d = null;
        if (this.cfg.stateful) {
        ...
        if (d) {
            ...
            this.expandedNodes = [];
            var a = this.headers.filter(".ui-state-active"), e = this.jq.find(".ui-menu-parent > .ui-menu-list:not(.ui-helper-hidden)");
...
PrimeFaces.widget.OrderList = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        if (!this.cfg.disabled) {
            ...
            this.setupButtons();
            this.bindEvents();
            this.list.sortable({revert: 1, start: function (c, d) {
                    PrimeFaces.clearSelection()
                }, update: function (c, d) {
                    b.onDragDrop(c, d)
                }})
        ...
        var c = this, e = c.list.children(".ui-orderlist-item.ui-state-highlight"), d = e.length, b = 0, a = e.is(":first-child");
        if (a) {
            return
        }
        e.each(function () {
        ...
        var d = this, f = d.list.children(".ui-orderlist-item.ui-state-highlight"), e = f.length, b = 0, a = f.is(":first-child"), c = f.eq(0).index();
        if (a) {
            return
        }
        f.each(function (h) {
            var i = $(this), g = (h === 0) ? 0 : (i.index() - c);
            if (!i.is(":first-child")) {
        ...
        var c = this, e = $(c.list.children(".ui-orderlist-item.ui-state-highlight").get().reverse()), d = e.length, b = 0, a = e.is(":last-child");
        if (a) {
            return
        }
        e.each(function () {
        ...
        var d = this, g = $(d.list.children(".ui-orderlist-item.ui-state-highlight").get().reverse()), f = g.length, c = 0, a = g.is(":last-child"), e = g.eq(0).index(), b = this.items.length;
        if (a) {
            return
        }
        g.each(function (i) {
            var j = $(this), h = (i === 0) ? b - 1 : (j.index() - e) - 1;
            if (!j.is(":last-child")) {
        ...
        if (this.hasBehavior("select")) {
            var c = this.cfg.behaviors.select, a = {params: [{name: this.id + "_itemIndex", value: b.index()}, {name: this.id + "_metaKey", value: d.metaKey}, {name: this.id + "_ctrlKey", value: d.ctrlKey}]};
            c.call(this, a)
        }
    }, fireItemUnselectEvent: function (c) {
        if (this.hasBehavior("unselect")) {
            var a = this.cfg.behaviors.unselect, b = {params: [{name: this.id + "_itemIndex", value: c.index()}]};
            a.call(this, b)
        }
...
PrimeFaces.widget.OutputPanel = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        var b = this, a = {source: this.id, process: this.id, update: this.id, async: true, ignoreAutoUpdate: true, global: this.cfg.global, params: [{name: this.id + "_load", value: true}], onsuccess: function (e, c, d) {
                PrimeFaces.ajax.Response.handle(e, c, d, {widget: b, handle: function (f) {
                        this.jq.html(f)
                    }});
                return true
        ...
        var e = $(window), d = e.scrollTop(), a = e.height(), c = this.jq.offset().top, b = c + this.jq.innerHeight();
...
PrimeFaces.widget.OverlayPanel = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        var b = this, a = c || this.cfg.target;
...
PrimeFaces.widget.Paginator = PrimeFaces.widget.BaseWidget.extend({
    init: function (b) {
        ...
        var a = this, b = this.pagesContainer.children(".ui-paginator-page");
        b.each(function () {
            var d = $(this), c = parseInt(d.text());
        ...
        var a = (this.cfg.rowCount === 0) ? 0 : (this.cfg.page * this.cfg.rows) + 1, c = (this.cfg.page * this.cfg.rows) + this.cfg.rows;
        ...
        for (var e = a; e <= b; e++) {
            var f = "ui-paginator-page ui-state-default ui-corner-all", j = this.cfg.ariaPageLabel.replace("{0}", (e + 1));
            if (this.cfg.page == e) {
                f += " ui-state-active"
            }
...
PrimeFaces.widget.PickList = PrimeFaces.widget.BaseWidget.extend({
    init: function (c) {
        ...
        var b = this.sourceList.prev(".ui-picklist-caption"), a = this.targetList.prev(".ui-picklist-caption");
        if (b.length) {
            this.sourceList.attr("aria-label", b.text())
        }
        if (a.length) {
            this.targetList.attr("aria-label", a.text())
        }
        ...
        if (this.cfg.disabled) {
            ...
            var e = this, d = true;
            $(this.jqId + " ul").sortable({cancel: ".ui-state-disabled,.ui-chkbox-box", connectWith: this.jqId + " .ui-picklist-list", revert: 1, update: function (f, g) {
                    e.unselectItem(g.item);
                    e.saveState();
                    if (d) {
                        e.fireReorderEvent();
                        d = false
                    ...
                    e.fireTransferEvent(g.item, g.sender, g.item.parents("ul.ui-picklist-list:first"), "dragdrop")
                }, start: function (f, g) {
                    e.itemListName = e.getListName(g.item);
                    e.dragging = true
                }, stop: function (f, g) {
                    e.dragging = false
                }, beforeStop: function (f, g) {
                    if (e.itemListName !== e.getListName(g.item)) {
                        d = false
                    } else {
                        d = true
                    }
        ...
        this.items.on("mouseover.pickList", function (c) {
            ...
            var l = $(this), j = l.parent(), g = (f.metaKey || f.ctrlKey);
            if (!f.shiftKey) {
                if (!g) {
                    a.unselectAll()
                }
                ...
                if (a.cursorItem && (a.cursorItem.parent().is(l.parent()))) {
                    var h = l.index(), m = a.cursorItem.index(), k = (h > m) ? m : h, d = (h > m) ? (h + 1) : (m + 1);
                    for (var c = k; c < d; c++) {
        ...
        var b = this, a = "ul.ui-picklist-source, ul.ui-picklist-target";
        this.jq.off("focus.pickList blur.pickList keydown.pickList", a).on("focus.pickList", a, null, function (d) {
            var c = $(this), f = b.focusedItem || c.children(".ui-state-highlight:visible:first");
            if (f.length) {
                b.focusedItem = f
            } else {
            ...
            var g = $(this), h = $.ui.keyCode, f = i.which;
            switch (f) {
                case h.UP:
                    b.removeOutline();
        ...
        var g = $.trim(h).toLowerCase(), f = e.children("li.ui-picklist-item"), b = this.isAnimated();
        if (g === "") {
            f.filter(":hidden").show()
        } else {
            for (var c = 0; c < f.length; c++) {
                var j = f.eq(c), a = j.attr("data-item-label"), d = this.filterMatcher(a, g);
                if (d) {
                    if (b) {
                        j.fadeIn("fast")
                    } else {
        ...
        if (this.hasBehavior("select")) {
            var c = this.cfg.behaviors.select, a = {params: [{name: this.id + "_itemIndex", value: b.index()}, {name: this.id + "_listName", value: this.getListName(b)}]};
            c.call(this, a)
        }
    }, fireItemUnselectEvent: function (c) {
        if (this.hasBehavior("unselect")) {
            var a = this.cfg.behaviors.unselect, b = {params: [{name: this.id + "_itemIndex", value: c.index()}, {name: this.id + "_listName", value: this.getListName(c)}]};
            a.call(this, b)
        }
    }, fireReorderEvent: function () {
        if (this.hasBehavior("reorder")) {
    ...
    init: function (a) {
        ...
        if (this.cfg.ajax) {
            this.progressPoll = setInterval(function () {
                var b = {source: a.id, process: a.id, formId: a.cfg.formId, global: a.cfg.global, async: true, oncomplete: function (f, c, d) {
                        var e = d[a.id + "_value"];
                        a.setValue(e);
                        if (e === 100) {
                            a.fireCompleteEvent()
...
PrimeFaces.widget.Resizable = PrimeFaces.widget.BaseWidget.extend({
    init: function (b) {
        ...
        if (this.cfg.behaviors) {
            ...
            if (a) {
                var b = {params: [{name: this.id + "_width", value: parseInt(d.helper.width())}, {name: this.id + "_height", value: parseInt(d.helper.height())}]};
                a.call(this, b)
            }
        }
    }});
        ...
        this.input.on("keydown.slider", function (f) {
            ...
            switch (c) {
                ...
                case d.TAB:
                    break;
                default:
                    var g = f.metaKey || f.ctrlKey, b = (c >= 48 && c <= 57) || (c >= 96 && c <= 105);
    ...
    init: function (a) {
        ...
        if ($.trim(a) === "") {
            ...
            if (this.cfg.prefix && a.indexOf(this.cfg.prefix) === 0) {
                ...
                if (this.cfg.suffix && a.indexOf(this.cfg.suffix) === (a.length - this.cfg.suffix.length)) {
                    a = a.substring(0, a.length - this.cfg.suffix.length)
                }
            }
            if (this.cfg.precision) {
                a = parseFloat(a)
...
PrimeFaces.widget.Spotlight = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        var c = $(document), b = $(document.body), e = this.target.offset();
        b.children("div.ui-spotlight-top").css({left: 0, top: 0, width: b.width(), height: e.top});
        var d = e.top + this.target.outerHeight();
        b.children("div.ui-spotlight-bottom").css({left: 0, top: d, width: b.width(), height: c.height() - d});
        b.children("div.ui-spotlight-left").css({left: 0, top: e.top, width: e.left, height: this.target.outerHeight()});
        var a = e.left + this.target.outerWidth();
        b.children("div.ui-spotlight-right").css({left: a, top: e.top, width: b.width() - a, height: this.target.outerHeight()})
    }, bindEvents: function () {
        var a = this;
        ...
        $(document).on("keydown." + this.id, function (d) {
            ...
            if (d.which === $.ui.keyCode.TAB) {
                ...
                if (c.length) {
                    var f = c.filter(":first"), b = c.filter(":last");
...
PrimeFaces.widget.Sticky = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        var d = this, c = $(window), b = "scroll." + this.cfg.id, a = "resize." + this.cfg.id;
        ...
        if (!this.fixed || a) {
            this.target.css({position: "fixed", top: this.cfg.margin, "z-index": ++PrimeFaces.zindex}).addClass("ui-shadow ui-sticky");
        ...
        if (this.fixed) {
            this.target.css({position: "static", top: "auto", width: "auto"}).removeClass("ui-shadow ui-sticky");
            this.ghost.remove();
            this.fixed = false
        }
    }});
PrimeFaces.widget.TabView = PrimeFaces.widget.DeferredWidget.extend({
    init: function (a) {
        ...
        var b = this, a = this.navContainer.children("li");
        ...
        var f = parseInt(this.navContainer.css("margin-left")), b = f + c, a = this.navscroller.innerWidth(), d = this;
        if (c < 0) {
            var e = this.lastTab.position().left + parseInt(this.lastTab.innerWidth());
            if (e > a) {
                this.navContainer.animate({"margin-left": b + "px"}, "fast", "easeInOutCirc", function () {
                    d.saveScrollState(b);
                    if ((e + c) < a) {
        ...
        var d = this, c = a.index(), b = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_contentLoad", value: true}, {name: this.id + "_newTab", value: a.attr("id")}, {name: this.id + "_tabindex", value: c}], onsuccess: function (h, f, g) {
                PrimeFaces.ajax.Response.handle(h, f, g, {widget: d, handle: function (i) {
                        a.html(i);
                        if (this.cfg.cache) {
                            this.markAsLoaded(a)
                        }
        ...
        var h = this.navContainer.children().eq(d), b = this.panelContainer.children().eq(d);
        h.remove();
        b.remove();
        ...
        if (f > 0) {
            if (d < this.cfg.selected) {
                ...
                if (d === this.cfg.selected) {
                    var e = (this.cfg.selected === (f)) ? (this.cfg.selected - 1) : this.cfg.selected, g = this.navContainer.children("li"), c = g.eq(e);
                    if (c.hasClass("ui-state-disabled")) {
        ...
        var c = this.cfg.behaviors.tabChange, b = {params: [{name: this.id + "_newTab", value: a.attr("id")}, {name: this.id + "_tabindex", value: a.index()}]};
        c.call(this, b)
    }, fireTabCloseEvent: function (d, a) {
        if (this.hasBehavior("tabClose")) {
            var c = this.cfg.behaviors.tabClose, b = {params: [{name: this.id + "_closeTab", value: d}, {name: this.id + "_tabindex", value: a}]};
            c.call(this, b)
        }
    }, hasBehavior: function (a) {
        if (this.cfg.behaviors) {
...
PrimeFaces.widget.Tooltip = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        if (this.cfg.trackMouse && this.mouseEvent) {
            this.jq.position({my: "left top+15", at: "right bottom", of: this.mouseEvent, collision: "flipfit"});
            ...
            this.jq.position({my: a, at: b, of: this.target, collision: "flipfit"})
        }
    }, show: function () {
        if (this.target) {
            var a = this;
        ...
        this.target.on("mousemove.tooltip-track", function (b) {
            a.jq.position({my: "left top+15", at: "right bottom", of: b, collision: "flipfit"})
        })
...
PrimeFaces.widget.BaseTree = PrimeFaces.widget.BaseWidget.extend({
    init: function (a) {
        ...
        if (this.cfg.dynamic) {
            ...
            var a = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_expandNode", value: c.getRowKey(b)}], onsuccess: function (g, e, f) {
                    PrimeFaces.ajax.Response.handle(g, e, f, {widget: c, handle: function (h) {
        ...
        if (this.isCheckboxSelection() && this.cfg.dynamic) {
            var e = this, a = {source: this.id, process: this.id};
            ...
            if (this.hasBehavior("select")) {
                var d = this.cfg.behaviors.select, b = {params: [{name: this.id + "_instantSelection", value: this.getRowKey(c)}]};
                d.call(this, b)
            }
        }
        ...
        if (this.hasBehavior("contextMenu")) {
            var b = this.cfg.behaviors.contextMenu, a = {params: [{name: this.id + "_contextMenuNode", value: this.getRowKey(c)}]};
            b.call(this, a)
        }
        ...
        if ($(f.target).is(":not(.ui-tree-toggler)")) {
            ...
            if (b && this.cfg.selectionMode) {
                var d = this.isNodeSelected(e), g = f.metaKey || f.ctrlKey, c = f.shiftKey;
                if (this.isCheckboxSelection()) {
...
PrimeFaces.widget.VerticalTree = PrimeFaces.widget.BaseTree.extend({
    init: function (a) {
        ...
        this.jq.off("keydown.tree blur.tree", ".ui-treenode-label").on("keydown.tree", ".ui-treenode-label", null, function (j) {
            if (!a.focusedNode) {
                return
            }
            var k = "", m = $.ui.keyCode;
            switch (j.which) {
                case m.LEFT:
                    var d = a.focusedNode.data("rowkey").toString(), n = d.length;
                    if (a.isExpanded(a.focusedNode)) {
                        ...
                        for (var h = 1; h < parseInt(n / 2) + 1; h++) {
                            ...
                            f = a.container.find("li:visible[data-rowkey = '" + k + "']");
                            if (f.length) {
                                a.focusNode(f);
                                break
                            }
                ...
                case m.RIGHT:
                    if (!a.focusedNode.hasClass("ui-treenode-leaf")) {
                        ...
                        if (!a.isExpanded(a.focusedNode) && !a.cfg.dynamic) {
                            k = d + "_0";
                            var f = a.container.find("li:visible[data-rowkey = '" + k + "']");
                            if (f.length) {
                                a.focusNode(f)
                            }
                        }
                ...
                case m.UP:
                    ...
                    if (b.length) {
                        f = b.find("li.ui-treenode:visible:last");
                        if (!f.length) {
                            f = b
                        }
                    } else {
                        f = a.focusedNode.closest("ul").parent("li")
                    }
                    if (f.length) {
                        a.focusNode(f)
                    }
                    ...
                    break;
                case m.DOWN:
                    var f = null, c = a.focusedNode.find("> ul > li:visible:first");
                    if (c.length) {
                        f = c
                    } else {
                        if (a.focusedNode.next().length) {
                            ...
                            var d = a.focusedNode.data("rowkey").toString();
                            if (d.length !== 1) {
                                f = a.searchDown(a.focusedNode)
                            }
                        }
                    }
                    if (f && f.length) {
                ...
                case m.SPACE:
                    if (a.cfg.selectionMode) {
                        var l = a.focusedNode.children(".ui-treenode-content").hasClass("ui-tree-selectable");
                        if (a.cfg.onNodeClick) {
                            a.cfg.onNodeClick.call(a, a.focusedNode, j)
                        }
                        if (l) {
                            var g = a.isNodeSelected(a.focusedNode);
        ...
        }).on("blur.tree", ".ui-treenode-label", null, function (b) {
            if (a.focusedNode) {
        ...
        var c = d + " .ui-tree-selectable", b = a.nodeType ? a.event + ".treenode." + a.nodeType : a.event + ".treenode", g = a.event + ".tree";
        ...
        var b = this, e = h.find("> .ui-treenode-content"), g = e.find("> .ui-tree-toggler"), d = h.data("nodetype"), c = g.nextAll("span.ui-treenode-icon"), a = this.cfg.iconStates[d], f = h.children(".ui-treenode-children");
        ...
        var d = f.find("> .ui-treenode-content"), e = d.find("> .ui-tree-toggler"), c = f.data("nodetype"), b = e.nextAll("span.ui-treenode-icon"), a = this.cfg.iconStates[c];
        ...
        b.draggable({helper: function () {
                var d = $('<div class="ui-tree-draghelper ui-state-highlight"></div>');
                d.width(c.jq.width());
                d.height(20);
                return d
            }, appendTo: document.body, zIndex: ++PrimeFaces.zindex, revert: true, scope: a}).data({dragsourceid: this.jqId, dragmode: this.cfg.dragMode})
    }, makeDropPoints: function (b) {
        var c = this, a = this.cfg.dragdropScope || this.id;
        b.droppable({hoverClass: "ui-state-hover", accept: "span.ui-treenode-content", tolerance: "pointer", scope: a, drop: function (e, n) {
                var f = $(n.draggable.data("dragsourceid")).data("widget"), m = n.draggable.data("dragmode"), l = c, p = $(this), o = p.closest("li.ui-treenode-parent"), q = c.getRowKey(o), i = n.draggable.closest("li.ui-treenode"), g = c.findTargetDragNode(i, m), k = c.getRowKey(g), d = g.next("li.ui-tree-droppoint"), r = g.parent().closest("li.ui-treenode-parent"), h = (f.id !== l.id);
                n.helper.remove();
                ...
                c.fireDragDropEvent({dragNodeKey: k, dropNodeKey: q, dragSource: f.id, dndIndex: p.prevAll("li.ui-treenode").length, transfer: h})
            }})
    }, makeDropNodes: function (b) {
        var c = this, a = this.cfg.dragdropScope || this.id;
        b.droppable({accept: ".ui-treenode-content", tolerance: "pointer", scope: a, over: function (d, e) {
                $(this).children(".ui-treenode-label").addClass("ui-state-hover")
            }, out: function (d, e) {
                $(this).children(".ui-treenode-label").removeClass("ui-state-hover")
            }, drop: function (e, o) {
                var f = $(o.draggable.data("dragsourceid")).data("widget"), m = o.draggable.data("dragmode"), l = c, n = $(this), p = n.closest("li.ui-treenode"), r = c.getRowKey(p), i = o.draggable.closest("li.ui-treenode"), g = c.findTargetDragNode(i, m), k = c.getRowKey(g), d = g.next("li.ui-tree-droppoint"), s = g.parent().closest("li.ui-treenode-parent"), q = p.children(".ui-treenode-children"), h = (f.id !== l.id);
                o.helper.remove();
                ...
                c.fireDragDropEvent({dragNodeKey: k, dropNodeKey: r, dragSource: f.id, dndIndex: g.prevAll("li.ui-treenode").length, transfer: h})
            }})
        ...
        this.jq.children("div.ui-tree-scroller").droppable({accept: ".ui-treenode-content", tolerance: "pointer", scope: a, over: function () {
        ...
        if (this.cfg.draggable) {
            a.data({dragsourceid: this.jqId, dragmode: this.cfg.dragMode})
        }
        ...
        var c = this;
        b.each(function (f) {
            var e = $(this), g = e.attr("data-rowkey"), d = (a === null) ? f.toString() : a + "_" + f;
            e.attr({id: c.id + ":" + d, "data-rowkey": d});
        ...
        var d = c.find("> .ui-treenode-content > .ui-chkbox"), a = c.find("> .ui-treenode-children > .ui-treenode");
        if (a.length) {
        ...
        var d = this, b = {source: this.id, process: c.transfer ? this.id + " " + c.dragSource : this.id};
        b.params = [{name: this.id + "_dragdrop", value: true}, {name: this.id + "_dragNode", value: c.dragNodeKey}, {name: this.id + "_dragSource", value: c.dragSource}, {name: this.id + "_dropNode", value: c.dropNodeKey}, {name: this.id + "_dndIndex", value: c.dndIndex}];
...
PrimeFaces.widget.HorizontalTree = PrimeFaces.widget.BaseTree.extend({
    ...
    }, focusNode: function (a) {}, partialCheck: function (d) {
        var b = d.children(".ui-chkbox-box"), a = b.children(".ui-chkbox-icon"), c = d.closest(".ui-treenode"), e = this.getRowKey(c);
...
PrimeFaces.widget.TreeTable = PrimeFaces.widget.DeferredWidget.extend({
    init: function (a) {
        ...
        this.sortableColumns.filter(".ui-state-active").each(function () {
            var c = $(this), d = c.children("span.ui-sortable-column-icon"), b = null;
        ...
        this.sortableColumns.on("mouseenter.treeTable", function () {
            ...
            if ($(d.target).is("th,span:not(.ui-c)")) {
                PrimeFaces.clearSelection();
                var c = $(this), b = c.data("sortorder") || "DESCENDING";
                if (b === "ASCENDING") {
                    b = "DESCENDING"
                } else {
                    if (b === "DESCENDING") {
        ...
        var e = this, b = {source: this.id, update: this.id, process: this.id, params: [{name: this.id + "_sorting", value: true}, {name: this.id + "_sortKey", value: d.attr("id")}, {name: this.id + "_sortDir", value: a}], onsuccess: function (h, f, g) {
                PrimeFaces.ajax.Response.handle(h, f, g, {widget: e, handle: function (i) {
                        this.tbody.html(i);
        ...
        var d = this, b = c.attr("data-rk"), a = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_expand", value: b}], onsuccess: function (h, f, g) {
                PrimeFaces.ajax.Response.handle(h, f, g, {widget: d, handle: function (i) {
        ...
        var b = this.getChildren(d), j = true, f = false, g = this.cfg.nativeElements ? d.find("> td:first-child > :checkbox") : d.find("> td:first-child > div.ui-chkbox > div.ui-chkbox-box > span.ui-chkbox-icon");
        for (var e = 0; e < b.length; e++) {
        ...
        if (this.isCheckboxSelection()) {
            ...
            if (this.hasBehavior("select")) {
                var d = this.cfg.behaviors.select, c = {params: [{name: this.id + "_instantSelection", value: b}]};
                d.call(this, c)
            }
        }
    }, fireUnselectNodeEvent: function (b) {
        if (this.hasBehavior("unselect")) {
            var a = this.cfg.behaviors.unselect, c = {params: [{name: this.id + "_instantUnselection", value: b}]};
            a.call(this, c)
        }
    }, setupScrolling: function () {
        ...
        var d = this.jq.parent().innerHeight() * (parseInt(this.cfg.scrollHeight) / 100), e = this.jq.children(".ui-treetable-header").outerHeight(true), b = this.jq.children(".ui-treetable-footer").outerHeight(true), c = (this.scrollHeader.outerHeight(true) + this.scrollFooter.outerHeight(true)), a = (d - (c + e + b));
        this.scrollBody.height(a)
    }, adjustScrollWidth: function () {
        var a = parseInt((this.jq.parent().innerWidth() * (parseInt(this.cfg.scrollWidth) / 100)));
        this.setScrollWidth(a)
        ...
        var a = this.thead.find("> tr > th > span.ui-column-resizer"), b = this;
        a.draggable({axis: "x", start: function () {
                if (b.cfg.liveResize) {
                    b.jq.css("cursor", "col-resize")
                } else {
                    var d = b.cfg.stickyHeader ? b.clone : b.thead, c = b.cfg.scrollable ? b.scrollBody.height() : d.parent().height() - d.height() - 1;
                    if (b.cfg.stickyHeader) {
                        c = c - b.relativeHeight
                    }
                    b.resizerHelper.height(c);
                ...
                var c = {source: b.id, process: b.id, params: [{name: b.id + "_colResize", value: true}, {name: b.id + "_columnId", value: e.attr("id")}, {name: b.id + "_width", value: e.width()}, {name: b.id + "_height", value: e.height()}]};
        ...
        this.showRowEditors(c);
        if (this.hasBehavior("rowEditInit")) {
            var b = this.cfg.behaviors.rowEditInit, d = c.data("rk");
            var a = {params: [{name: this.id + "_rowEditIndex", value: d}]};
            b.call(this, a)
        }
    }, showRowEditors: function (a) {
        ...
        var f = a.closest("tr"), g = f.data("rk"), b = f.hasClass("ui-expanded-row"), e = this, c = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_rowEditIndex", value: g}, {name: this.id + "_rowEditAction", value: d}], onsuccess: function (j, h, i) {
                PrimeFaces.ajax.Response.handle(j, h, i, {widget: e, handle: function (k) {
                        if (b) {
                            this.collapseRow(f)
                        }
        ...
        var e = a.children(".ui-cell-editor"), f = e.attr("id"), d = a.index(), c = a.closest("tr").data("rk") + "," + d, g = this;
        var b = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_cellInfo", value: c}, {name: f, value: f}], onsuccess: function (j, h, i) {
                PrimeFaces.ajax.Response.handle(j, h, i, {widget: g, handle: function (k) {
...
PrimeFaces.widget.Wizard = PrimeFaces.widget.BaseWidget.extend({
    init: function (b) {
        ...
        var d = this, a = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_wizardRequest", value: true}, {name: this.id + "_stepToGo", value: c}], onsuccess: function (g, e, f) {
                PrimeFaces.ajax.Response.handle(g, e, f, {widget: d, handle: function (h) {
                        this.content.html(h)
                    }});
                return true
        ...
        if (b) {
            a.params.push({name: this.id + "_backRequest", value: true})
        }
        PrimeFaces.ajax.Request.handle(a)
